...
def test_node_uninstaller_removes_only_managed_node_tools() -> None:
    script = read("scripts/uninstall-node.sh")

    assert "/usr/local/sbin/snell-fwctl" in script
    assert "/usr/local/lib/snell-ufw-control" in script
    assert "/etc/sudoers.d/snell-ufw-control" in script
    assert "REMOVE_USER" in script
    assert "userdel -r" in script
    assert "systemctl stop snell" not in script
    assert "ufw delete" not in script
