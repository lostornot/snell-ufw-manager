...
async def api_create_node(
    ...
    await db.add_op_log(node_id, name, "ADD_NODE", host.strip(), f"手动添加端口 {snell_port}")
...
async def api_discover_node(
    ...
    await db.add_op_log(node_id, hostname, "DISCOVER", host.strip(), f"自动发现并添加节点，默认端口: {final_snell_port}")
