...
The controller uses a local-only session model, not a multi-user account system.

- `SESSION_SECRET` is generated by `install-controller.sh` unless explicitly provided.
- The first local page visit creates a signed session cookie automatically.
...
- Read-only routes do not need CSRF.
- Failed CSRF checks must not store submitted secrets.
